...
export const Form = ({onAdd}) => {
   ...
   const [formData, setFormData] =useState({
    name : '',
    age  : '',
    nationality: '', // ADD THIS - missing field
    email: '',
    contactnumber: '',
    gender: '',
    address: '',
   ...
   const [errors,setErrors] = useState({
    ...
    switch (name) {
      case 'name':
        if (!value.trim()) error = '* Name is required'; // ADD REQUIRED CHECK
        if (/\d/.test(value)) error = '* Name must not contain numbers';
        break;
      case 'age':
        if (!value) error = '* Age is required'; // ADD REQUIRED CHECK
        if (value <= 0 || value > 120) error = '* Age must be between 1 and 120';
        break;
      case 'email':
        if (!value.trim()) error = '* Email is required'; // ADD REQUIRED CHECK
        if (!/^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/.test(value)) error = '* Invalid email format';
        break;
      case 'contactnumber':
        if (!value) error = '* Contact Number is required'; // ADD REQUIRED CHECK
...
const handleSubmit = (e) => {
  ...
  const updatedRecords = [...existing, userWithId]; // USE userWithId instead of formData
        ...
        {item.type === "textarea" ? (
          ...
          <select id={item.name} name={item.name} value={formData[item.name]} className={item.name} onChange={handleChange}>
            {item.options.map((option, i) => (
              <option key={i} value={option.value}> {/* ADD value={option.value} */}
            {option.text}
              </option>
            ))}
          </select>
